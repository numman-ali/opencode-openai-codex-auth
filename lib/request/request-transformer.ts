...
export function normalizeModel(model: string | undefined): string {
	...
	// 1. GPT-5.2 Codex (newest codex model - supports xhigh reasoning)
	if (
	...
	// 2. GPT-5.2 general purpose (supports same reasoning as Codex Max)
...
export function getReasoningConfig(
	...
	const isCodex = normalizedName.includes("codex") && !isCodexMini && !isGpt52Codex;
	const isLightweight =
		!isCodexMini &&
	...
	// - Codex models (including GPT-5.2 Codex) do NOT support "none" - only low/medium/high/xhigh
	...
	// GPT-5.2 Codex and Codex Max default to "high" as they support xhigh
	const defaultEffort: ReasoningConfig["effort"] = isCodexMini
		? "medium"
		: isGpt52Codex || isCodexMax
			? "high"
			: supportsXhigh
				? "high"
				: isLightweight
	...
	// Codex CLI presets are low/medium/high (or xhigh for Codex Max and GPT-5.2 Codex)
	if ((isCodex || isGpt52Codex) && effort === "minimal") {
		effort = "low";
	}
